...
; Reference: http://www.ccs.neu.edu/home/dorai/t-y-scheme/t-y-scheme-Z-H-7.html#node_sec_5.1
(define counter 0)
...
(define (get-solution solutions N) 
  (cond [(equal? #() solutions)  (move-queen (random N) (build-start-vector N) N)]
        [(full-solution? N solutions N) solutions]
        [(= counter maxsteps) (display "Either no solution or exceed maxsteps.")]
        [else (move-queen (random N) solutions N)])) 


(define (build-start-vector N)
  (do ((vec (make-vector (+ N 1)))
       (i 0 (+ i 1))
       (x 0 (if (= (modulo (+ x 2) N) 0) N (modulo (+ x 2) N))))
    ((> i N) vec)
    (vector-set! vec i x)))
#|
(define (build-start-vector N)
...
(define (move-queen col solutions N)
  (let [(row (vector-ref solutions col))]
    (cond [(= 0 col) (get-solution solutions N)]
          [(= 0 (conflict-sum row col solutions N)) (get-solution solutions N)]
...
(define (nq-mc N) 
  [begin
    ...
    (display N)
    (newline)
    (display "Min-conflict: Columns are selected by random")
    (newline)
